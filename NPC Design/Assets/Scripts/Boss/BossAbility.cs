...
public class BossAbility : MonoBehaviour
{
    ...
    public enum StatusEffect { None, Stagger, KnockUp, KnockBack }
    ...
    [field: SerializeField] public float abilityRange { get; private set; }
    ...
    {
        if(bossUsingThisAbility.playerToTarget != null)
        {
            if(Vector3.Distance(bossUsingThisAbility.playerToTarget.transform.position, bossUsingThisAbility.transform.position) > abilityRange)
            {
