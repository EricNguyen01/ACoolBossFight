...
public class AbilityDamageTriggerCollider : MonoBehaviour
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        if (damageable != null)
        {
            IDamageable.DamageEffect damageEffect = IDamageable.DamageEffect.None;

            if (statusEffect == BossAbility.StatusEffect.Stagger) damageEffect = IDamageable.DamageEffect.Stagger;
            if (statusEffect == BossAbility.StatusEffect.KnockUp) damageEffect = IDamageable.DamageEffect.KnockUp;

            if (damageable.GetColliderEnabledStatus()) damageable.GetDamaged(Random.Range(minDamage, maxDamage), damageEffect);
        }
    }
